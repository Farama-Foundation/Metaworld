...
class SawyerSoccer6DOFEnv(SawyerXYZEnv):
    def __init__(
            self,
            random_init=False,
            obs_type='plain',
            tasks = [{'goal': np.array([0., 0.9, 0.03]),  'obj_init_pos':np.array([0, 0.6, 0.03]), 'obj_init_angle': 0.3}], 
...
if __name__ == '__main__':  
    import time 
    env = SawyerSoccer6DOFEnv(random_init=True)    
    for _ in range(1000):   
        env.reset()
        for _ in range(50):
            env.render()
