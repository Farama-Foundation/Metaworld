...
class SawyerPlateSlideBack6DOFEnv(SawyerXYZEnv):
    def __init__(
            self,
            random_init=False,
            obs_type='plain',
            ...
            tasks = [{'goal': np.array([0., 0.6, 0.015]),  'obj_init_pos':np.array([0., 0.85, 0.02]), 'obj_init_angle': 0.3}], 
...
if __name__ == '__main__':  
    import time 
    env = SawyerPlateSlideBack6DOFEnv(random_init=True)    
    for _ in range(1000):   
        env.reset()
        for _ in range(50):
            env.render()
