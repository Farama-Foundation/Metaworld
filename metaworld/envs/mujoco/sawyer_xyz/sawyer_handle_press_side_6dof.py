...
class SawyerHandlePressSide6DOFEnv(SawyerXYZEnv):
    def __init__(
            self,
            random_init=True,
            obs_type='plain',
            # tasks = [{'goal': np.array([0, 0.78, 0.12]), 'obj_init_pos':np.array([0., 0.75, 0.12]), 'obj_init_qpos':0.}], 
            tasks = [{'goal': np.array([-0.2, 0.7, 0.14]), 'obj_init_pos':np.array([-0.3, 0.7, 0.05])}], 
            goal_low=None,
            goal_high=None,
            rotMode='fixed',#'fixed',
            multitask=False,
...
if __name__ == '__main__':  
    import time 
    env = SawyerHandlePressSide6DOFEnv(random_init=True)    
    for _ in range(1000):   
        env.reset()
        for _ in range(10):
