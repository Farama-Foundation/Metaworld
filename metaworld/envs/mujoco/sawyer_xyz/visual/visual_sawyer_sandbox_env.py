...
class VisualSawyerSandboxEnv(SawyerXYZEnv):
    ...
    def reset_model(self):
        ...
        overlapping_voxels = (self._world.mat > 1).sum()
        shape = self._world.mat.shape
        total_voxels = shape[0] * shape[1] * shape[2]
        print(f'Percentage of voxels containing overlaps: {overlapping_voxels / total_voxels}')

        # img = np.rot90(self._world.mat, 1)
        # img = img.astype('uint8')
        # img[img == 1] = 0
        # img = img.sum(axis=2)
        # cv2.imwrite('img.png', img * 255 / img.max())
