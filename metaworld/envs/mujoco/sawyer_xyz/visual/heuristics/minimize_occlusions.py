...
class MinimizeOcclusions(Heuristic):
    # TODO This doesn't account for the fact that the camera looks out at the
    # world radially. It only ensures that object heights increase with Y coord

    def __init__(self, tries=30):
        self._tries = tries

    def apply_to(self, tool, voxel_space, rg):
        res = voxel_space.resolution
        tool_height = int((tool.bbox[5] - tool.bbox[2]) * res)

        # TODO I'm pretty sure this should be tool_height: but that doesn't work
        covered = voxel_space.mat[:, :, tool_height:].sum(axis=2).astype('bool')
        idx_first_tool = np.argmax(covered, axis=0)
        for j in range(covered.shape[1]):
            covered[:idx_first_tool[j], j] = True
