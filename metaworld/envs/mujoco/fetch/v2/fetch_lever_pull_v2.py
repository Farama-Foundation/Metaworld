...
class FetchLeverPullEnvV2(FetchEnv):
    """FetchLeverPullEnv.
    ...
    @property
    def model_name(self):
        return full_v2_path_for("fetch/fetch_lever_pull.xml")
    ...
    def compute_reward(self, action, obs):
        ...
        # De-emphasize y error so that we get Fetch's shoulder underneath the
        # lever prior to bumping on against
        scale = np.array([4.0, 1.0, 4.0])
        # Offset so that we get the Fetch's shoulder underneath the lever,
...
class TrainLeverPullv2(FetchLeverPullEnvV2):
    tasks = None

    def __init__(self):
        FetchLeverPullEnvV2.__init__(self, self.tasks)

    def reset(self, seed=None, options=None):
        return super().reset(seed=seed, options=options)


class TestLeverPullv2(FetchLeverPullEnvV2):
    tasks = None
