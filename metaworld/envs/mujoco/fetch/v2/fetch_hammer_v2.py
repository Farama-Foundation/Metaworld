...
class FetchHammerEnvV2(FetchEnv):
    ...
    @property
    def model_name(self):
        return full_v2_path_for("fetch/fetch_hammer.xml")
    ...
    def compute_reward(self, actions, obs):
        ...
        threshold = FetchHammerEnvV2.HAMMER_HANDLE_LENGTH / 2.0
        if abs(hammer[0] - hand[0]) < threshold:
            hammer_threshed[0] = hand[0]

        reward_quat = FetchHammerEnvV2._reward_quat(obs)
        ...
        reward_in_place = FetchHammerEnvV2._reward_pos(hammer_head, self._target_pos)
...
class TrainHammerv2(FetchHammerEnvV2):
    tasks = None

    def __init__(self):
        FetchHammerEnvV2.__init__(self, self.tasks)

    def reset(self, seed=None, options=None):
        return super().reset(seed=seed, options=options)


class TestHammerv2(FetchHammerEnvV2):
    tasks = None
