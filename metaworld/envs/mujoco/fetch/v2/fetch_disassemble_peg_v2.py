...
from metaworld.envs.mujoco.fetch.fetch_env import (
    FetchEnv,
    _assert_task_is_set,
)


class FetchNutDisassembleEnvV2(FetchEnv):
    ...
    @property
    def model_name(self):
        return full_v2_path_for("fetch/fetch_assembly_peg.xml")
    ...
    def compute_reward(self, actions, obs):
        ...
        threshold = FetchNutDisassembleEnvV2.WRENCH_HANDLE_LENGTH / 2.0
        if abs(wrench[0] - hand[0]) < threshold:
            wrench_threshed[0] = hand[0]

        reward_quat = FetchNutDisassembleEnvV2._reward_quat(obs)
        ...
        reward_in_place = FetchNutDisassembleEnvV2._reward_pos(
...
class TrainDisassemblev2(FetchNutDisassembleEnvV2):
    tasks = None

    def __init__(self):
        FetchNutDisassembleEnvV2.__init__(self, self.tasks)

    def reset(self, seed=None, options=None):
        return super().reset(seed=seed, options=options)


class TestDisassemblev2(FetchNutDisassembleEnvV2):
    tasks = None
