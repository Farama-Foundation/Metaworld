...
class FetchNutAssemblyEnvV2(FetchEnv):
    ...
    def compute_reward(self, actions, obs):
        ...
        threshold = FetchNutAssemblyEnvV2.WRENCH_HANDLE_LENGTH / 2.0
        if abs(wrench[0] - hand[0]) < threshold:
            wrench_threshed[0] = hand[0]

        reward_quat = FetchNutAssemblyEnvV2._reward_quat(obs)
        ...
        reward_in_place, success = FetchNutAssemblyEnvV2._reward_pos(
...
class TrainAssemblyv2(FetchNutAssemblyEnvV2):
    tasks = None

    def __init__(self):
        FetchNutAssemblyEnvV2.__init__(self, self.tasks)

    def reset(self, seed=None, options=None):
        return super().reset(seed=seed, options=options)


class TestAssemblyv2(FetchNutAssemblyEnvV2):
    tasks = None
