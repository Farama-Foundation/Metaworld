...
from gym import logger as logging
...
def qpos_from_site_pose(env,
  ...
  Args:
    ...
    inplace: (optional) If True, `env.sim.data` will be modified in place.
      Default value is False, i.e. a copy of `env.sim.data` will be made.
  Returns:
  ...
  if target_pos is not None and target_quat is not None:
    jac = np.empty((6*env.sim.model.nv), dtype=dtype)
    err = np.empty(6, dtype=dtype)
    jac_pos, jac_rot = jac[:3*env.sim.model.nv], jac[3*env.sim.model.nv:]
    err_pos, err_rot = err[:3], err[3:]
  else:
    jac = np.empty((3*env.sim.model.nv), dtype=dtype)
  ...
  site_xpos = env.sim.data.site_xpos[site_id]
  site_xmat = env.sim.data.site_xmat[site_id]
  ...
  if joint_names is None:
    ...
    joint_ids = [env.sim.model.joint_name2id(joint_name) for joint_name in joint_names]
    ...
    indexer = env.sim.model.dof_jntid
    ...
    dof_indices = np.array([indexer[i] in joint_ids for i in range(indexer.shape[0])])
  else:
  ...
  for steps in range(max_steps):
    ...
    if err_norm < tol:
      ...
      mjlib.mj_jacSite(
          env.sim.model, env.sim.data, jac_pos, jac_rot, site_id)
      jac_joints = jac.reshape(-1, env.sim.model.nv)[:, dof_indices]
      ...
      # Update `env.qpos`, taking quaternions into account.
      mjlib.mj_integratePos(env.sim.model, env.sim.data.qpos, update_nv, 1)
  ...
  if not success and steps == max_steps - 1:
    logging.warn('Failed to converge after %i steps: err_norm=%3g',
                    steps, err_norm)

  if not inplace:
    # TODO: try implementing this
    # Our temporary copy of env.sim.data is about to go out of scope, and when
    # it does the underlying mjData pointer will be freed and env.sim.data.qpos
    # will be a view onto a block of deallocated memory. We therefore need to
    # make a copy of env.sim.data.qpos while env.sim.data is still alive.
    qpos = env.sim.data.qpos.copy()
  else:
    # If we're modifying env.sim.data in place then it's fine to return a view.
