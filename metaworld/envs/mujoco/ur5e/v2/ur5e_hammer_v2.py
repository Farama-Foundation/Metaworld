...
class UR5eHammerEnvV2(UR5eEnv):
    ...
    @property
    def model_name(self):
        return full_v2_path_for("ur5e/ur5e_hammer.xml")
    ...
    def compute_reward(self, actions, obs):
        ...
        threshold = UR5eHammerEnvV2.HAMMER_HANDLE_LENGTH / 2.0
        if abs(hammer[0] - hand[0]) < threshold:
            hammer_threshed[0] = hand[0]

        reward_quat = UR5eHammerEnvV2._reward_quat(obs)
        ...
        reward_in_place = UR5eHammerEnvV2._reward_pos(hammer_head, self._target_pos)
...
class TrainHammerv2(UR5eHammerEnvV2):
    tasks = None

    def __init__(self):
        UR5eHammerEnvV2.__init__(self, self.tasks)

    def reset(self, seed=None, options=None):
        return super().reset(seed=seed, options=options)


class TestHammerv2(UR5eHammerEnvV2):
    tasks = None
