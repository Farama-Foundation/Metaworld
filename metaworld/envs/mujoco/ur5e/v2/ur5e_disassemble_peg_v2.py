...
class UR5eNutDisassembleEnvV2(UR5eEnv):
    ...
    @property
    def model_name(self):
        return full_v2_path_for("ur5e/ur5e_assembly_peg.xml")
    ...
    def compute_reward(self, actions, obs):
        ...
        threshold = UR5eNutDisassembleEnvV2.WRENCH_HANDLE_LENGTH / 2.0
        if abs(wrench[0] - hand[0]) < threshold:
            wrench_threshed[0] = hand[0]

        reward_quat = UR5eNutDisassembleEnvV2._reward_quat(obs)
        ...
        reward_in_place = UR5eNutDisassembleEnvV2._reward_pos(
...
class TrainDisassemblev2(UR5eNutDisassembleEnvV2):
    tasks = None

    def __init__(self):
        UR5eNutDisassembleEnvV2.__init__(self, self.tasks)

    def reset(self, seed=None, options=None):
        return super().reset(seed=seed, options=options)


class TestDisassemblev2(UR5eNutDisassembleEnvV2):
    tasks = None
