...
class SawyerReachXYZ6DOFEnv(SawyerXYZEnv, MultitaskEnv):
    def __init__(
            self,
        ...
        SawyerXYZEnv.__init__(self, model_name=self.model_name, frame_skip=frame_skip, **kwargs)
        ...
        self.action_space = Box(np.array([-1, -1, -1, 0, -1, -1, -1]), np.array([1, 1, 1, np.pi*2, 1, 1, 1]), dtype=np.float32)
        self.hand_space = Box(self.hand_low, self.hand_high, dtype=np.float32)
        self.observation_space = Box(np.concatenate((self.hand_low, self.hand_low)), np.concatenate((self.hand_high, self.hand_high)), dtype=np.float32)
    ...
    def _get_obs(self):
        flat_obs = np.concatenate([self.get_endeff_pos(),
                                    self._state_goal])
        return flat_obs
...
class SawyerReachXY6DOFEnv(SawyerReachXYZ6DOFEnv):
