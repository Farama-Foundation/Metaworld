...
class SawyerSweepIntoGoalEnv(MultitaskEnv, SawyerXYZEnv):
    def __init__(
            self,
            puck_low=(-.4, .2),
            puck_high=(.4, 1),
            ...
            reset_free=False,
            xml_path='sawyer_xyz/sawyer_table_with_hole.xml',
        ...
        self.observation_space = Box(
            np.hstack((self.hand_low, self.hand_low)),
            np.hstack((self.hand_high, self.hand_high)),
            dtype=np.float32
        )
    ...
    def _get_info(self):
        ...
        # puck distance
        puck_diff = puck_goal - self.get_puck_pos()
        ...
        # state distance
        state_diff = np.hstack((self.get_endeff_pos(), self.get_puck_pos())) - self._state_goal
    ...
    def compute_rewards(self, actions, obs):
        ...
        r = np.array([-touch_distances - puck_distances])
        return r
