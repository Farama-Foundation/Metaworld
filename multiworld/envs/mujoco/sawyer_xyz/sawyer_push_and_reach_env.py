...
class SawyerPushAndReachXYZEnv(MultitaskEnv, SawyerXYZEnv):
	def __init__(
		...
		MultitaskEnv.__init__(self, norm_order)
	...
	def _get_info(self):
		...
		puck_distance = np.linalg.norm(puck_goal - self.get_puck_pos()[:2], ord=self.norm_order)
		hand_distance_l2 = np.linalg.norm(hand_goal - self.get_endeff_pos(), ord=2)
		puck_distance_l2 = np.linalg.norm(puck_goal - self.get_puck_pos()[:2], ord=2)
		touch_distance = np.linalg.norm(
			self.get_endeff_pos() - self.get_puck_pos(),
			ord=self.norm_order,
		)
		state_distance = np.linalg.norm(
			np.hstack((self.get_endeff_pos(), self.get_puck_pos()[:2])) - self._state_goal,
			ord=self.norm_order,
		)
		return dict(
